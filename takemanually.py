...
def manually_fill():
    ...
    def fill_attendance():
        ...
        if subb == "":
            ...
            def create_csv():
                # Create Attendance(Manually) directory if it doesn't exist
                manual_attendance_dir = os.path.join(BASE_DIR, "Attendance(Manually)")
            ...
            # Fixed: Changed function name to avoid conflict
                ...
                if os.path.exists(manual_attendance_path):
                    try:
                        if os.name == 'nt':  # Windows
                            ...
                            os.system(f'open "{manual_attendance_path}"' if os.name == 'posix' else f'xdg-open "{manual_attendance_path}"')
                    except Exception as e:
                        print(f"Error opening folder: {e}")
                else:
                    print("Attendance folder not found")

            # Fixed: Changed variable name and fixed syntax error
            attf_button = tk.Button(  # Changed from 'attf' to 'attf_button'
                MFW,
                text="Check Sheets",  # Fixed: 'texts' → 'text'
                command=open_attendance_folder,  # Fixed: function name
                fg="black",
                bg="lawn green",
                width=12,
                height=1,
            ...
            attf_button.place(x=730, y=410)  # Fixed: variable name
